...
var DeviceIpAdress = "192.168.1.128";
...
function main() {
    // Vars
    var DeviceIpAdress = adapter.config.host;
    const cmd = "/getjp"; // Kommandos in der URL nach der Host-Adresse
    var statusuz ="on";
	var numinv = 0;
	var data='{"608":null}';
...
function test() {
	adapter.getState("info.numinv", function (err, obj) {
		...
		adapter.log.debug("Anzahl positiv: " + testi);
			if (testi==numinv){
				adapter.log.debug("Alle gefunden");
			...
			testi=0;
			console.log("nicht alle gefunden");
			testj++;
			  if (testj>3){
				console.log("Fehler, noch nicht alle Unterzähler angelegt");
				clearInterval(testend);}
			}
		}	
	});
...
function check(uz) {   
    ...
    else{adapter.log.debug("Adapter " + uz + " nicht vorhanden");
...
// create States PAC/Status/StatusSE Inverter(i)
    ...
    adapter.setObjectNotExists('INV.' + (dataJuz[141][i.toString()][119]) + ".statusSE",{
        type: 'state',
        common: {
            name: 'statusSE',
